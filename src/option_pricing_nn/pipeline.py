...
from option_pricing_nn.dataset import FEATURE_COLUMNS, generate_synthetic_dataset, save_dataset
...
def run_experiment(config: ExperimentConfig) -> dict[str, float]:
    ...
    joblib.dump(artifacts.target_scaler, config.paths.output_dir / "target_scaler.joblib")
    save_metrics(metrics, config.paths.metrics_dir / "nn_metrics.json")

    test_df = pd.DataFrame(artifacts.scaler.inverse_transform(artifacts.x_test), columns=FEATURE_COLUMNS)
    ...
    save_metrics(mc_metrics, config.paths.metrics_dir / "monte_carlo_vs_black_scholes_metrics.json")

    plot_loss(artifacts.history, config.paths.figure_dir / "loss.png")
    plot_true_vs_predicted(artifacts.y_test, y_pred, config.paths.figure_dir / "true_vs_predicted.png")
    ...
    plot_error_against_feature(test_df["t"].to_numpy(), errors, "Maturity T", config.paths.figure_dir / "error_vs_maturity.png")
    plot_error_against_feature(
